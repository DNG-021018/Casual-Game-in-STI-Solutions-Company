...
public class VT_UISfxPool : MonoBehaviour
{
    ...
    IEnumerator ReturnWhenDone(AudioSource s, float time, Action onDone = null)
    {
    ...
    internal void PlayAtTime(AudioClip clip, float time, float finalVolume)
    {
        AudioSource s = Get();
        s.clip = clip;
        ...
        s.enabled = true;
        s.gameObject.SetActive(true);
        s.PlayScheduled(AudioSettings.dspTime + time);

        StartCoroutine(ReturnWhenDone(s, time, null));
    }
}
