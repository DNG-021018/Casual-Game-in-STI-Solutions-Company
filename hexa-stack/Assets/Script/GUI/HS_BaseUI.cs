...
namespace HexaStack
{
    public abstract class HS_BaseUI : MonoBehaviour
    {
        [System.Serializable]
        struct Entry
        {
            public UIPageId id;
            public HS_UIPage page;
        }
        [SerializeField] List<Entry> pages = new();

        protected readonly Dictionary<UIPageId, HS_UIPage> _map = new();
        ...
        {
            if (HS_GameManager.Instance) HS_GameManager.Instance.OnGameStateChanged += HandleGameState;
        }
        ...
        {
            if (!_map.TryGetValue(id, out HS_UIPage page)) return;
            StopAllCoroutines();
            StartCoroutine(CoOpen(id, page, ctx, clearStack));
        }

        IEnumerator CoOpen(UIPageId id, HS_UIPage page, object ctx, bool clearStack)
        {
        ...
        IEnumerator CoBack()
        {
            HS_UIPage cur = _map[_stack.Pop()];
            yield return StartCoroutine(cur.Hide());

            HS_UIPage prev = _map[_stack.Peek()];
