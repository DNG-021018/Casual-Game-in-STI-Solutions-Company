...
{
    public abstract class DZ_BaseUI : MonoBehaviour
    {
        [System.Serializable]
        struct Entry
        {
            public UIPageId id;
            public DZ_UIPage page;
        }
        [SerializeField] List<Entry> pages = new();

        protected readonly Dictionary<UIPageId, DZ_UIPage> _map = new();
        protected readonly Stack<UIPageId> _stack = new();
        protected DZ_UIManager uiManager;
        ...
        {
            if (!_map.TryGetValue(id, out DZ_UIPage page)) return;
            StopAllCoroutines();
            StartCoroutine(CoOpen(id, page, ctx, clearStack));
        }

        IEnumerator CoOpen(UIPageId id, DZ_UIPage page, object ctx, bool clearStack)
        {
        ...
        IEnumerator CoBack()
        {
            DZ_UIPage cur = _map[_stack.Pop()];
            yield return StartCoroutine(cur.Hide());

            DZ_UIPage prev = _map[_stack.Peek()];
