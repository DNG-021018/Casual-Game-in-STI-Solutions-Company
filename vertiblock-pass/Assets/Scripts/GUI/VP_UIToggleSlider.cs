...
{
    public class VP_UIToggleSlider : MonoBehaviour, IPointerClickHandler
    {
        [Header("Slider setup")]
        ...
        private void SetStateAndStartAnimation(bool state)
        {
            ...
            if (_previousValue != CurrentValue)
            {
                ...
                {
                    onToggleOn?.Invoke();
                    if (OnSprite != null) OnSprite.gameObject.SetActive(false);
                }
                else
                {
                    if (OnSprite != null) OnSprite.gameObject.SetActive(true);
                    onToggleOff?.Invoke();
                }
            }
            AnimateSlider();
