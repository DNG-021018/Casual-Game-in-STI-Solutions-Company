...
{
    public abstract class VP_BaseUI : MonoBehaviour
    {
        [System.Serializable]
        struct Entry
        {
            public UIPageId id;
            public VP_UIPage page;
        }
        [SerializeField] List<Entry> pages = new();

        protected readonly Dictionary<UIPageId, VP_UIPage> _map = new();
        ...
        {
            if (VP_GameManager.Instance) VP_GameManager.Instance.OnGameStateChanged += HandleGameState;
        }
        ...
        {
            if (!_map.TryGetValue(id, out VP_UIPage page)) return;
            StopAllCoroutines();
            StartCoroutine(CoOpen(id, page, ctx, clearStack));
        }

        IEnumerator CoOpen(UIPageId id, VP_UIPage page, object ctx, bool clearStack)
        {
        ...
        IEnumerator CoBack()
        {
            VP_UIPage cur = _map[_stack.Pop()];
            yield return StartCoroutine(cur.Hide());

            VP_UIPage prev = _map[_stack.Peek()];
