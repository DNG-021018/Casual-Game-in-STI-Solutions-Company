...
{
    public class CS_TutorialPage : CS_UIPage
    {
        [Header("Panel")]
        [SerializeField] Panels TutorialPanel;

        [Header("Buttons")]
        [SerializeField] CS_UIButton nextBtn;
        [SerializeField] CS_UIButton prevBtn;
        [SerializeField] CS_UIButton exitBtn;
        [SerializeField] CS_UIHorizontalPager pager;

        CS_BaseUI parent;

        Vector2 _menuStart;

        public override void Init(CS_BaseUI p)
        {
            base.Init(p);
            parent = p;
