...
{
    public abstract class CS_BaseUI : MonoBehaviour
    {
        [System.Serializable]
        struct Entry
        {
            public UIPageId id;
            public CS_UIPage page;
        }
        [SerializeField] List<Entry> pages = new();

        protected readonly Dictionary<UIPageId, CS_UIPage> _map = new();
        ...
        {
            if (CS_GameManager.Instance) CS_GameManager.Instance.OnGameStateChanged += HandleGameState;
        }
        ...
        {
            if (!_map.TryGetValue(id, out CS_UIPage page)) return;
            StopAllCoroutines();
            StartCoroutine(CoOpen(id, page, ctx, clearStack));
        }

        IEnumerator CoOpen(UIPageId id, CS_UIPage page, object ctx, bool clearStack)
        {
        ...
        IEnumerator CoBack()
        {
            CS_UIPage cur = _map[_stack.Pop()];
            yield return StartCoroutine(cur.Hide());

            CS_UIPage prev = _map[_stack.Peek()];
